...
class ResNet_hr(nn.Module):
    ...
    def forward(self, x, ape=True):
        ...
        x = self.relu(x)
        if ape:
            #print(self.pos_embed1)
            x = x + F.interpolate(self.pos_embed1, size=(x.size(2), x.size(3)), mode='bicubic')
        c2 = self.layer1(x)
        c3 = self.layer2(c2)
        c4 = self.layer3(c3)
        c5 = self.layer4(c4)
...
def resnet18(**kwargs):
    return ResNet_hr(BasicBlock, [2, 2, 2, 2], **kwargs)    
