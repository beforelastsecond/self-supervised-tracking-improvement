...
def main():
    ...
    if args.pretrain:
        if os.path.isfile(args.pretrain):
            ...
            checkpoint['state_dict']['colorizer.refs_queue'] = torch.randint(low=0, high=10000,
                                                                             size=(args.bsize * args.factor,))
            ...
            if args.freeze_all:  # 冻结训练时只训练注意力部分
    ...
    for epoch in range(epoch, args.epochs):
        ...
        TrainImgLoader = torch.utils.data.DataLoader(
            train_dataset, batch_size=args.bsize // torch.distributed.get_world_size(), num_workers=args.worker,
